export function mount(container) {
  // 1. Inject the SVG structure
  container.innerHTML = `
    <svg id="svg" width="100%" height="100vh">
      <defs>
        <linearGradient id="grad" x1="50%" y1="100%" x2="50%" y2="10%">
          <stop offset="0%" style="stop-color: #1B3347; stop-opacity:1"/>
          <stop offset="100%" style="stop-color: #172C3C;stop-opacity:1"/>
        </linearGradient>
        <radialGradient id="test">
          <stop offset="40%" style="stop-color: #fff;"/>
          <stop offset= "70%" style="stop-color: #23435c;"/>
        </radialGradient>
        <filter id="shadow" width="200" height="200">
          <feOffset result="offOut" in="SourceGraphic" dx="1" dy="1"/>
          <feGaussianBlur result="blurOut" in="offOut" stdDeviation="3"/>
          <feBlend in="SourceGraphic" in2="blurOut" mode="normal"/>
        </filter>
      </defs>
    </svg>
  `;

  // 2. JS Logic
  const svgNS = "http://www.w3.org/2000/svg";
  const svg = container.querySelector("#svg");
  
  let randomX = 0, randomY = 0, randomR = 0, randomDelay = 0;
  let ViewportX = window.innerWidth, ViewportY = window.innerHeight;
  let listItems = Array.from(container.querySelectorAll(".list-item"));

  const pathArr = [
    {
      id: 0,
      path: createPathStr([74, 34.4, "L", 65.1, 26.8, "L", 68.1, 18.69, "L", 74.8, 24.9, "Z", "M", 74, 34.4, "L", 76.49, 43.2, "L", 78.4, 51.9, "L", 83.35, 55.2]),
      description: "In fact this stars' sequence is an asterism within Ursa Major. that is now the third largest of the 88 modern constellations. Also The Big Dipper is used as a symbol of north as well as flag of Alaska.",
    },
    {
      id: 1,
      path: createPathStr([71.61, 83.66, "L", 74.47, 75.95, "L", 77.34, 78.68, "L", 73.95, 87.43, "Z", "M", 71.61, 83.6, "L", 66.66, 89.07, "L", 61.45, 89.07, "L", 56.77, 86.88]),
      description: "The Ursa Minor has been one of the most important constellation. for navigation as it contains the Polaris being as North Star",
    },
    {
      id: 2,
      path: createPathStr([62.5, 12.56, "L", 46.875, 21.3, "L", 36.45, 27.86, "L", 28.125, 39.89, "M", 46.875, 21.30, "L", 47.65, 30, "L", 49.2, 36]),
      description: "This constellation was named in honor of. Cassiopeia's daughter — Andromeda, in the Greek myth.",
    },
    {
      id: 3,
      path: createPathStr([19.27, 56.3, "L", 21.875, 50.8, "L", 20.3125, 46.45, "L", 16.66, 45.9, "L", 14, 45.4, "L", 13.28, 51.91 ,"L", 12.76, 61.75, "L", 9.65, 74.68, "L", 8.59, 77.6, "L", 6.8, 78.69, "M", 6.8, 78.69, "L", 4.7, 89.57, "M", 6.8, 78.69, "L", 3.39, 88.8, "M", 6.8, 78.69, "L", 2.3, 83.6, "M", 6.8, 78.69, "L", 2.35, 83.6, "M", 6.8, 78.69, "L", 2.3, 75.96, "M", 6.8, 78.69, "L", 2.1, 71.58]),
      description: "Scorpio is one of the brightest constellations in the sky",
    },
    {
      id: 4,
      path: createPathStr([73.96, 45.36, "L", 77.6, 36.6, "L", 72.9, 33.3, "L", 72.1, 39.3, "Z", "M", 73.96, 45.36, "L", 73.96, 67.2, "L", 73.4, 78.1, "L", 68.75, 77, "L", 67.2, 68.3, "L", 66.4, 54.1, "L", 64.3, 44.8, "L", 62, 36.6, "L", 57.3, 36.1, "L", 45.3, 50.8, "L", 44, 54.1, "L", 40.1, 70.5, "L", 40.1, 86.3, "L", 45.3, 92.9]),
      description: "One of the Drago's star called Kepler-10 has. Kepler-10b — smallest and closest to Earth exoplanet",
    },
    {
      id: 5,
      path: createPathStr([58.3, 7.1, "L", 60.2, 16.4, "L", 60.7, 20.21, "L", 60.2, 25.1, "L", 59.6, 26.8, "L", 57.3, 30, "L", 55.5, 31.7, "L", 42.7, 26.8, "L", 38.5, 29, "M", 57.3, 30, "L", 58.9, 35, "M", 58.9, 35, "L", 66.1, 33.3 , "L", 65.6, 29.5, "L", 66.4, 26.2, "L", 69.5, 20.2, "L", 70.6, 19.1, "L", 72.6, 18, "M", 58.9, 35, "L", 57.8, 41, "L", 59.1, 50.8, "L", 64.6, 59.6, "M", 59.1, 50.8, "L", 53.6, 55.2, "L", 52.6, 65, "M", 53.6, 55.2, "L", 47.9, 53.5, "L", 44.8, 57.4]),
      description: "Sagittarius is one of the constellations of the zodiac. The center of the Milky Way lies in the westernmost part of Sagittarius",
    }
  ];

  function createPathStr(pArr) {
    let path = 'M';
    let index = 1;
    pArr.forEach(function(p) {
      if (typeof p === "number") {
        if (index % 2 == 0) {
          path += " " + (ViewportY - ((p / 100) * ViewportY)).toFixed(0);
          index++;
        } else {
          path += " " + (ViewportX - ((p / 100) * ViewportX)).toFixed(0);
          index++;
        }
      } else {
        path += " " + p;
      }
    });
    return path;
  }

  function setCoords(c, rX, rY, fill, isRandom) {
    c.setAttribute("cx", rX);
    c.setAttribute("cy", rY);
    c.setAttribute("fill", fill);
    c.setAttribute("r", Number.isInteger(isRandom) ? 2 : isRandom);
    svg.appendChild(c);
  }

  function setProps(el, classArr, attrArr) {
    if (el) {
      if (classArr && classArr.length) {
        classArr.forEach(clas => el.classList.add(clas));
      }
      if (attrArr && attrArr.length) {
        attrArr.forEach(attr => el.setAttribute(attr.style, attr.val));
      }
    }
  }

  function setGroupCoords(group, rX, rY) {
    group.setAttribute("transform", "translate(" + rX + "," + rY + ")");
    svg.appendChild(group);
  }

  function setPath(pathStr) {
    let pathEl = document.createElementNS(svgNS, "path");
    setProps(pathEl, [], [
      { style: "id", val: "constellation" },
      { style: "fill", val: "none" },
      { style: "stroke", val: "#fff" },
      { style: "style", val: "opacity: .5;" },
      { style: "stroke-width", val: 2 },
      { style: "d", val: pathStr }
    ]);
    svg.appendChild(pathEl);
    
    // Quick hack since getTotalLength isn't always available immediately without DOM attachment, 
    // but in modern browsers it is if attached. We wait a tiny bit to get length correctly.
    setTimeout(() => {
        try {
            let total_length = pathEl.getTotalLength();
            pathEl.setAttribute("stroke-dasharray", total_length);
            pathEl.setAttribute("stroke-dashoffset", total_length);
        } catch(e) {}
    }, 0);
  }

  function setDesc(pathDesc) {
    let descEl = document.createElementNS(svgNS, "text");
    let paragraphs = pathDesc.split(".");
    let x = (ViewportX - ((67.54 / 100) * ViewportX)).toFixed(0);
    let y = (ViewportY - ((67.3 / 100) * ViewportY)).toFixed(0);
    let verticalIndent = 20;

    for (let i = 0; i < paragraphs.length; i++) {
      if (paragraphs[i].length) {
        let tspan = document.createElementNS(svgNS, "tspan");
        let indentX = i < 1 ? 0 : -10;
        let txtNode = document.createTextNode(paragraphs[i].trim() + ".");
        tspan.appendChild(txtNode);
        setProps(tspan, [], [{ style: "y", val: verticalIndent }, { style: "x", val: indentX }]);
        descEl.appendChild(tspan);
        verticalIndent += 25;
      }
    }
    descEl.classList.add("desc");
    descEl.setAttribute("transform", "translate(" + x + "," + y + ")");
    svg.appendChild(descEl);
  }

  function drawStars() {
    let allCoords = [];
    let n = (ViewportX >= 1000 && ViewportY >= 600) ? 40 : 20;
    let n1 = (ViewportX >= 1000 && ViewportY >= 600) ? 400 : 200;

    for (let i = 0; i < 300; i++) {
      let circle = document.createElementNS(svgNS, "circle");
      randomX = +(Math.random() * ViewportX).toFixed(2);
      randomY = +(Math.random() * ViewportY).toFixed(2);
      randomR = +(Math.random() * 2.5).toFixed(2);
      setCoords(circle, randomX, randomY, "#fff", randomR);
      allCoords.push(circle);
    }

    for(let i = 0; i < n1; i++) {
      let circle = document.createElementNS(svgNS, "circle");
      randomX = +(Math.random() * ViewportX).toFixed(2);
      randomY = +(Math.random() * ViewportY).toFixed(2);
      randomDelay = +(Math.random() * (-4 - 2.5) + -4).toFixed(2);
      circle.classList.add("animate");
      circle.style.animationDelay = randomDelay + "s";
      setCoords(circle, randomX, randomY, "#fff", 2);
    }

    allCoords.forEach(function(el) {
      let elX = +el.attributes["cx"].value;
      let elY = +el.attributes["cy"].value;
      for (let i = 0; i < allCoords.length; i++) {
        let arrElX = +allCoords[i].attributes["cx"].value;
        let arrElY = +allCoords[i].attributes["cy"].value;
        if ((elY - arrElY <= n && elY - arrElY >= -n) && (elX - arrElX <= n && elX - arrElX >= -n)) {
          let path = document.createElementNS(svgNS, "path");
          let pathStr = "M " + elX + " " + elY + " L " + arrElX + " " + arrElY;
          setProps(path, [], [
            { style: "fill", val: "none" },
            { style: "stroke", val: "#fff" },
            { style: "style", val: "opacity: .05;" },
            { style: "stroke-width", val: 1 },
            { style: "d", val: pathStr }
          ]);
          svg.appendChild(path);
        }
      }
    });
  }

  function deleteEl() {
    let path = svg.querySelector("#constellation");
    let text = svg.querySelector(".desc");
    let c = svg.querySelectorAll(".animate");
    for (let i = 0; i < c.length; i++) c[i].classList.remove("paused");
    if (path) svg.removeChild(path);
    if (text) svg.removeChild(text);
    svg.querySelectorAll(".star-group").forEach(el => svg.removeChild(el));
  }

  let timeoutId;
  function drawPath(e) {
    let id = +e.currentTarget.id;
    let currentPathObj = pathArr.find(obj => obj.id === id);
    if (!currentPathObj) return;

    let pathStr = currentPathObj.path;
    let pathDesc = currentPathObj.description;
    let newCoords = [];
    
    setPath(pathStr);
    
    let c = svg.querySelectorAll(".animate");
    for (let i = 0; i < c.length / 1.4; i++) c[i].classList.add("paused");
    
    clearTimeout(timeoutId);
    timeoutId = setTimeout(() => {
      for(let i = 0; i < c.length; i++) c[i].classList.remove("paused");
    }, 3000);

    let pathRegex = /\\w\\s\\d+\\s\\d+/gi;
    let filterM = /M\\s\\d+\\s\\d+/g;
    let cleanCoords = /[MLZ]\\s/gi;
    
    let coordsArr = pathStr.match(/[A-Z]?\s?[\d\.]+\s[\d\.]+/g) || [];
    coordsArr.forEach(coord => {
      let cleaned = coord.replace(/[a-zA-Z]\s?/, '').trim();
      if (cleaned) newCoords.push(cleaned);
    });

    setDesc(pathDesc);

    for (let i = 0; i < newCoords.length; i++) {
        let parts = newCoords[i].split(' ');
        if(parts.length < 2) continue;
        let X = parseFloat(parts[0]);
        let Y = parseFloat(parts[1]);
        
        let group = document.createElementNS(svgNS, "g");
        let circle = document.createElementNS(svgNS, "circle");
        randomR = +(Math.random() * (4 - 1.5) + 1.5).toFixed(2);
        setProps(circle, ["cStar"], [
          { style: "stroke-width", val: 10 },
          { style: "stroke", val: "rgba(225,225,225,.1)" },
          { style: "r", val: randomR }
        ]);
        setProps(group, ["star-group"], []);
        group.appendChild(circle);
        setGroupCoords(group, X, Y);
    }
  }

  drawStars();

  listItems.forEach(el => {
    el.addEventListener("mouseover", drawPath);
    el.addEventListener("mouseout", deleteEl);
  });

  return () => {
    clearTimeout(timeoutId);
    listItems.forEach(el => {
      el.removeEventListener("mouseover", drawPath);
      el.removeEventListener("mouseout", deleteEl);
    });
    container.innerHTML = '';
  };
}
